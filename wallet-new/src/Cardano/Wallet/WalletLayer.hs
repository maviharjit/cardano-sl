...
deriveGeneric ''ImportWalletError

instance HasDiagnostic ImportWalletError where
    getDiagnosticKey = \case
        ImportWalletFileNotFound _          -> "unknownAccount"
        ImportWalletNoWalletFoundInBackup _ -> "message"
        ImportWalletCreationFailed _        -> "message"
...
instance ToJSON ImportWalletError where
    toJSON = jsendErrorGenericToJSON

instance FromJSON ImportWalletError where
    parseJSON = jsendErrorGenericParseJSON
-- | Unsound show instance needed for the 'Exception' instance.
...
instance Exception ImportWalletError where
    toException   = walletExceptionToException
    fromException = walletExceptionFromException
